...
class Individual:
    ...
    def calculate_fitness(self):
        ...
        col_score = sum(len(set(self.board[r][c] for r in range(6))) for c in range(6))
        subgrid_score = 0
...
class GeneticAlgorithm:
    ...
    def initialize_population(self):
        for _ in range(self.population_size):
            ...
            for r in range(6):
                empty_cells = [c for c in range(6) if board[r][c] == 0]
                for c in empty_cells:
                    possible_values = set(range(1, 7))
                    for i in range(6):
                        possible_values.discard(board[r][i])
                        possible_values.discard(board[i][c])
                    start_row, start_col = (r // 2) * 2, (c // 3) * 3
    ...
    def mutate(self, individual):
        for r in range(6):
            if random.random() < self.mutation_rate:
                empty_cells = [c for c in range(6) if self.puzzle[r][c] == 0]
...
# Example 6x6 puzzle
SPG = SudokuPuzzleGenerator(size=6)
solution = SPG.generate_full_solution()
puzzle = SPG.remove_values_to_create_puzzle(solution, clues=10)
...
# Plotting the fitness score over generations
